...
<li class="btn btn-outline-warning" onclick="rellenarciudad('<% out.print(rs.getString(1)); %>', '<% out.print(rs.getString(2)); %>')">
...
<%
        ...
        try {
            PreparedStatement psConsulta = conn.prepareStatement("SELECT * FROM ciudades WHERE ciu_nombre = ?");
            psConsulta.setString(1, nombre);
            ResultSet rsConsulta = psConsulta.executeQuery();

            if (rsConsulta.next()) {
                out.println("<br>");
                out.println("<i class='alert alert-danger'>La ciudad ya existe.</i>");
            } else {
                PreparedStatement psInsertar = conn.prepareStatement("INSERT INTO ciudades(ciu_nombre) VALUES (?)");
                ...
                if (filasInsertadas > 0) {
                    out.println("<br>");
                    out.println("<i class='alert alert-success'>Ciudad insertado correctamente.</i>");
                } else {
                    out.println("<br>");
        ...
        try {
            ...
            PreparedStatement psSelect = conn.prepareStatement("SELECT ciu_nombre FROM ciudades WHERE idciudad = ?");
            psSelect.setInt(1, idciudad);
            ResultSet rs = psSelect.executeQuery();

            // Verificar si la ciudad existe
            if (rs.next()) {
                String nombreActual = rs.getString("ciu_nombre");
            ...
            PreparedStatement psModificar = conn.prepareStatement("UPDATE ciudades SET ciu_nombre = ? WHERE idciudad = ?");
        ...
        try {
            ...
            PreparedStatement psEliminar = conn.prepareStatement("DELETE FROM ciudades WHERE idciudad = ?");
