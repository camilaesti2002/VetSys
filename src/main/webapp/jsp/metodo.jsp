...
<li class="btn btn-outline-warning" onclick="rellenarmetodo('<% out.print(rs.getString(1)); %>', '<% out.print(rs.getString(2)); %>')">
...
<%
        ...
        try {
            PreparedStatement psConsulta = conn.prepareStatement("SELECT * FROM metododepago WHERE nombre = ?");
            psConsulta.setString(1, nombre);
            ResultSet rsConsulta = psConsulta.executeQuery();

            if (rsConsulta.next()) {
                out.println("<br>");
                out.println("<i class='alert alert-danger'>El metodo ya existe.</i>");
            } else {
                PreparedStatement psInsertar = conn.prepareStatement("INSERT INTO metododepago(nombre) VALUES (?)");
        ...
        try {
            ...
            int idmetodo = Integer.parseInt(pk);
            ...
            PreparedStatement psSelect = conn.prepareStatement("SELECT nombre FROM metododepago WHERE idmetodo= ?");
            psSelect.setInt(1, idmetodo);
            ...
            if (rs.next()) {
                ...
                out.println("<br>");
                out.println("<i class='alert alert-danger'>No se encontró un metodo con el ID proporcionado.</i>");
                return;  // Salir del método porque el ID de ciudad no existe
            }

            // Preparar la consulta de actualización
            PreparedStatement psModificar = conn.prepareStatement("UPDATE metododepago SET nombre = ? WHERE idmetodo = ?");
            psModificar.setString(1, nombre);
            psModificar.setInt(2, idmetodo); // Usar setInt para establecer el ID de la ciudad
        ...
        try {
            ...
            PreparedStatement psEliminar = conn.prepareStatement("DELETE FROM metododepago WHERE idmetodo = ?");
            psEliminar.setInt(1,idmetodo);
