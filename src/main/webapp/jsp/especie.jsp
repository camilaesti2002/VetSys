...
<%    if (request.getParameter("listar").equals("listar")) {
        try {
            ...
            rs = st.executeQuery("select * from especie order by idespecies asc");
            while (rs.next()) {
%>
<tr>
...
<li class="btn btn-outline-warning" onclick="rellenarespecie('<% out.print(rs.getString(1)); %>', '<% out.print(rs.getString(2)); %>')">
...
<%
            ...
            out.println("Error SQL al listar especie: " + e.getMessage());
        }
    } else if (request.getParameter("listar").equals("cargar")) {
        String nombre = request.getParameter("nombre");

        try {
            PreparedStatement psConsulta = conn.prepareStatement("SELECT * FROM especie where nombre = ?");
            psConsulta.setString(1, nombre);
            ResultSet rsConsulta = psConsulta.executeQuery();

            if (rsConsulta.next()) {
                out.println("<br>");
                out.println("<i class='alert alert-danger'>La especie ya existe.</i>");
            } else {
                PreparedStatement psInsertar = conn.prepareStatement("INSERT INTO especie(nombre) VALUES (?)");
                ...
                if (filasInsertadas > 0) {
                    out.println("<br>");
                    out.println("<i class='alert alert-success'>Especie insertado correctamente.</i>");
                } else {
                    out.println("<br>");
            ...
            out.println("<br>");
            out.println("Error SQL al insertar especie: " + e.getMessage());
        }
        ...
        try {
            ...
            int idciudad = Integer.parseInt(pk);
            ...
            PreparedStatement psSelect = conn.prepareStatement("SELECT nombre FROM especie WHERE idespecies = ?");
            psSelect.setInt(1, idciudad);
            ...
            PreparedStatement psModificar = conn.prepareStatement("UPDATE especie SET nombre = ? WHERE idespecies = ?");
            psModificar.setString(1, nombre);
            psModificar.setInt(2, idciudad); // Usar setInt para establecer el ID de la ciudad
            ...
            if (filasModificadas > 0) {
                out.println("<br>");
                out.println("<i class='alert alert-success'>Ciudad actualizada correctamente.</i>");
            } else {
                out.println("<br>");
        ...
        try {
            ...
            PreparedStatement psEliminar = conn.prepareStatement("DELETE FROM especie WHERE idespecies = ?");
            psEliminar.setInt(1, idciudad);

            int filasEliminadas = psEliminar.executeUpdate();

            if (filasEliminadas > 0) {
                out.println("<br>");
                out.println("<i class='alert alert-success'>Ciudad eliminada correctamente.</i>");
            } else {
                out.println("<br>");
            ...
            if (e.getSQLState().equals("23503")) {
                out.println("<br>");
                out.println("<i class='alert alert-danger'>Esta ciudad se utiliza en un registro.</i>");
            } else {
                out.println("<br>");
